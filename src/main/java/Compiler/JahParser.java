...
public class JahParser {
    static private String buffer;

    /* LIST OF ALL TOKENS */

    final static private Pattern whitespace = Pattern.compile("\\s+");
    final static private Pattern entryidentifier = Pattern.compile(".*((Lets go)|(Ok)|(Alright)|(So)|(DJ \\w+)).*[,!]\\n");
    final static private Pattern identifier = Pattern.compile("[^\\d&&[^\\s]][\\w&&[^\\s]]*");
    final static private Pattern intdefinition = Pattern.compile("(the)|(a)|(them)|(they)|(her)|(his)|(their)|(my)[\\s]+[^\\d&&[^\\s]][\\w&&[^\\s]]*");
    ...
    final static private Pattern openbracket = Pattern.compile("(thsen)");
    final static private Pattern closebracket = Pattern.compile("(yeah)|(yup)");
    final static private Pattern operator = Pattern.compile("(and)|(with)|(without)|(give it to)|(into)|(has)");
    final static private Pattern singlelinecomment = Pattern.compile("(gon)|(gonna)|(y*u+h+)|(wo+)|(ok)|(alright)");
    ...
    public static Queue<Token> parse(File file) throws FileNotFoundException {
        ...
        while(buffer.length() != 0) {
            ...
            if (singleline.lookingAt()) {
                ...
                tokens.offer(new Token("entry", Type.STRING, "entry"));
